...
#counter is "\SQLServer:Databases(_Total)\Transactions/sec"

$counter = "\SQLServer:Databases(_Total)\Transactions/sec"
...
$all= (get-counter -ComputerName $computername `
 ...
 select path , cookedValue
      ...
      foreach($PerformanceCounter in $all)
      {
          ...
          $InsertSQL+="INSERT INTO PerfTableSQLTransaction(Computer,Category,Label,Value,CaptureTime)
