...
#counter is "\SQLServer:Buffer Manager\Buffer cache hit ratio"

$counter = "\SQLServer:Buffer Manager\Buffer cache hit ratio"
 ...
 select path , cookedValue
     ...
     # $time = $currentTime.ToShortTimeString()
      ...
      foreach($PerformanceCounter in $all)
      {
          ...
          $InsertSQL+="INSERT INTO PerfTableSQLBufferCacheHit (Computer,Category,Label,Value,Date,Time)
