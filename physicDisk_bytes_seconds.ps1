...
#counter is "\PhysicalDisk(_Total)\Disk Bytes/sec"
...
$all= (get-counter -ComputerName $computername `
 ...
 select path , cookedValue
      ...
      foreach($PerformanceCounter in $all)
      {
          ...
          $realvalue=$PerformanceCounter.CookedValue /1024/1024
          ...
          $Label = "disk Mbytes/sec"
          #$value = $realvalue.ToString()
          #compose a batch insert from each records
          $InsertSQL+="INSERT INTO PerfTableDiskThroughput (Computer,Category,Label,Value,CaptureTime)
