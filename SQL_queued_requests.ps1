...
#counter is "\SQLServer:Workload Group Stats(default)\Queued requests"

$counter = "\SQLServer:Workload Group Stats(default)\Queued requests"
 ...
 select path , cookedValue
     ...
     # $time = $currentTime.ToShortTimeString()
      ...
      foreach($PerformanceCounter in $all)
      {
          ...
          $InsertSQL+="INSERT INTO PerfTableSQLQueuedRequests(Computer,Category,Label,Value,Date,Time)
          VALUES('$ip','$Category','$Label','$realvalue','$date','$currentTime');"
