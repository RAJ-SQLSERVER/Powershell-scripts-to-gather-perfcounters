...
#counter is "\Processor Information(_Total)\% Processor Time"

$counter = "\Processor Information(_Total)\% Processor Time"
$computername = get-content C:\users\wangchenxu3\servernamebim.txt
...
$all= (get-counter -ComputerName $computername `
 ...
 select path , cookedValue
      ...
      foreach($PerformanceCounter in $all)
      {
          ...
          $InsertSQL+="INSERT INTO PerfTableBIMUATCPU (Computer,Category,Label,Value,CaptureTime)
